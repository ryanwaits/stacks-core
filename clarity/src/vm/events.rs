...
pub(crate) fn hex_value(value: &Value) -> Result<String, SerializationError> {
    let mut byte_serialization = Vec::new();
...
mod tests {
    ...
    #[test]
    fn vm_trace_json_uses_array_order_not_event_index() {
        let contract_id = QualifiedContractIdentifier::new(
            StandardPrincipalData::null_principal(),
            ContractName::try_from("test-contract".to_string()).unwrap(),
        );
        let event = VmTraceEvent::Storage(StorageEvent::VarSet(
            VarSetEventData::try_from_value(contract_id, "n".into(), &Value::UInt(1)).unwrap(),
        ));
        let json = event.json_serialize(&"deadbeef", true);
